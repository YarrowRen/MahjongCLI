...
- **牌谱回放** - 浏览历史对局，上帝视角逐步回放每一个动作（主菜单选项 7）
...
游戏默认中文启动，语言、时间控制与 AI 速度均可在主菜单**设置**（选项 6）中修改；**历史回放**（选项 7）可逐步查看任意已完成对局。
...
后续所有版本同样由 Claude Code 实现，包括：多语言界面与 PyPI 发布（v1.1.0）、A+B 计时系统与设置菜单（v1.1.x）、天凤 1000 局真实牌谱规则验证、2026-06 全面引擎审查（修复天和/抢杠/北抜き宝牌/立直棒守恒等缺陷、清理死代码、架构分层、LRU 缓存提速）、逐步牌谱回放功能，以及 UI 改进（修复 AI 立直标记、赤5橙色显示、鸣/荣目标牌高亮、重排结算画面）。
